...
public abstract class AbstractCfRecommendationService implements RecommendationService {
  ...
  private EnumMap<SimilarityMetric, Recommender> recommendationTypeMap;
  ...
  private void autoRefresh() throws TasteException {
    ...
    long start = System.nanoTime();
    for (Map.Entry<SimilarityMetric, Recommender> RecommenderEntry : recommendationTypeMap.entrySet()) {
  ...
  protected Recommender getRecommender(SimilarityMetric similarityMetric) throws TasteException {
    if (recommendationTypeMap != null) {
      return (recommendationTypeMap.get(similarityMetric) != null) ?
              recommendationTypeMap.get(similarityMetric) : putAndReturnRecommender(similarityMetric);
    } else {
      recommendationTypeMap = new EnumMap<>(SimilarityMetric.class);
  ...
  private Recommender putAndReturnRecommender(SimilarityMetric similarityMetric) throws TasteException {
    recommendationTypeMap.put(similarityMetric, getRecommenderBuilder(similarityMetric).buildRecommender(dataModel));
    return recommendationTypeMap.get(similarityMetric);
  }
